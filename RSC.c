...
int g_cache_fp1;
FILE* g_log_fp;
char g_rscfile[PATH_MAX];
char g_rscfile1[PATH_MAX];
char g_logfile[PATH_MAX];
off_t g_cblksize;
...
//	PX_ASSERT(fdatasync(g_cache_fp) != -1);
...
void Write_cache_off(void* buf, off_t size, off_t off)
{
	...
	ssize_t nwritten, expectwritten;
#ifdef USEFREAD
	PX_ASSERT(fseek(g_cache_fp, off, SEEK_SET) == 0);
...
{
	char str[MAX_MSG];
	sprintf(str, "masterblock info:\n"
				"	file_header_section %ld\n"
				"	file_header_flist %ld\n"
				"	file_header_flist_tail %ld\n"
				"	block_pointer_section %ld\n"
				"	block_pointer_flist %ld\n"
				"	block_pointer_flist_tail %ld\n"
				"	block_section %ld\n"
				"	capacity %ld\n"
				"	cblksize %ld\n", HEADERINDEX(mb.file_header_section),
				HEADERINDEX(mb.file_header_flist), HEADERINDEX(mb.file_header_flist_tail),
				BLOCKHEADERINDEX(mb.block_pointer_section), BLOCKHEADERINDEX(mb.block_pointer_flist), BLOCKHEADERINDEX(mb.block_pointer_flist_tail), BLOCKINDEX(mb.block_section),
				mb.capacity, mb.cblksize);
	if (prstr == NULL)
	{
		printf(str);
	}
...
extern void cmlog(void *cmctx, const char *fmt, int line, const char *func, ...);
...
#else
	char str[MAX_MSG];
	sprintf(str, "RSC: %s :-%s file %s line %d function %s msg:%s \n", buf, msg_type,
			file, line, func, msg);
	cmlog(NULL, str, 0, NULL);
#endif
}
